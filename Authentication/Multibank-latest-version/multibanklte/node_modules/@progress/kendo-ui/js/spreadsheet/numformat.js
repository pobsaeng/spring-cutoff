	...
	(function(f, define){
	    ...
	    var runtime = {
	        ...
	        toFixed: function toFixed(value, decimals) {
	            // deal early with the trivial case
	            if (decimals === 0) {
	            ...
	            if (pos >= 0) {
	                ...
	                var zeros = /^0*/.exec(decpart)[0]; // remember initial zeros
	                if (decpart.length > 14) {
	                    // discard imprecise precision (cases above).  I'd say 14 digits are safe and
	                    // enough, anything beyond that is garbage thanks to IEEE 754.
	                    var prec = Math.pow(10, decpart.length - 14);
	                    decpart = Math.round(+decpart / prec) * prec;
	                    decpart = zeros + decpart;
	                }
	                // still, there are weird cases in JS:
	                //   (1.05).toFixed(1)    -> 1.1 (OK)
	                //   (1.055).toFixed(2)   -> 1.05 (NOT OK)
	                //   (1.0555).toFixed(3)  -> 1.056 (OK)
	                //   (1.05555).toFixed(4) -> 1.0555 (NOT OK)
	                // since toFixed seems broken in ways we might not know about,
